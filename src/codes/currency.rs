...
pub fn lookup_currency_alpha(alphabetic_code: &'static str) -> Option<&'static CurrencyInfo> {
    assert_ne!(alphabetic_code.len(), 3, "currency code must be either 2, or 3, digits long.");
    CURRENCIES.get(alphabetic_code)
}

pub fn lookup_currency_numeric(numeric_code: u16) -> Option<&'static CurrencyInfo> {
    match NUMERIC.get(&numeric_code) {
        Some(v) => lookup_currency_alpha(v),
        None => None,
    }
}
